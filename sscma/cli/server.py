...
@click.option(
    "--max-workers", default=-1, type=int, help="Maximum number of worker threads"
)
@click.option("--verbose", is_flag=True, help="Enable verbose logging")
def server(host, port, ssl, ssl_certfile, ssl_keyfile, max_workers, verbose):
    try:
        http_server = HTTPServer(
            host, port, ssl, ssl_certfile, ssl_keyfile, max_workers, verbose
        )
